...
var stacks=[];var color_schemes=[];var default_colors=['#AC4A08','#6C88A1','#D9DCC6','#EB9E1A','#FBCC37'];var contexts=[];var global_rotate=0;var current_history=[];return{define:{canvas:function(){},stack:function(){var colors=[];},defaultColors:function(new_default){default_colors=new_default;}},setupCanvas:function(target){var canvas=document.getElementById(target);var context=canvas.getContext('2d');var this_context_pair=[target,context];contexts.push(this_context_pair);},create:{canvas:function(parent,width,height){},stack:function(){var new_stack=[];var new_layer=[];var new_history=[];new_layer.push(new_history);new_stack.push(new_layer);stacks.push(new_stack);},layer:function(stack){if(!stack||isNaN(stack)){stack=0;}
var new_layer=[];var new_history=[];new_layer.push(new_history);stacks[stack].push(new_layer);},history:function(stack,layer){if(!stack||isNaN(stack)){stack=0;}
if(!layer||isNaN(layer)){layer=0;}
var new_history=[];stacks[stack][layer].push(new_history);}},clear:{layer:function(stack,layer){if(!stack||isNaN(stack)){stack=0;}
if(!layer||isNaN(layer)){layer=0;}
var target_layer=stacks[stack][layer];for(l=0;l<target_layer.length;l++){target_layer[l]=[];}}},collect:{current_history:function(){return current_history;}},convertTo:{PNG:function(target){var image_file=document.getElementById(target);image_file=image_file.toDataURL();return image_file;}},addToHistory:function(stack,layer,history_step,directive,information){if(!stack||isNaN(stack)){stack=0;}
if(!layer||isNaN(layer)){layer=0;}
if(!history_step||isNaN(history_step)){history_step=0;}
var new_history=[];new_history.push(directive,information);stacks[stack][layer][history_step].push(new_history);},interpretElement:function(element){var target=document.getElementById(element).tagName;return target;},findContext:function(element){var context='';for(c=0;c<contexts.length;c++){if(contexts[c][0]==element){context=contexts[c][1];}}
if(context===''){artisan.setupCanvas(element);context=artisan.findContext(element);}
return context;},drawCircle:function(target,placex,placey,radius,fill_color,line_width,stroke_color,alpha,shadow_blur,shadow_color,shadow_offset_x,shadow_offset_y){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;if(placex&&placex!=='random'){placex=artisan.analyzeValue(placex,target_width);}
if(placey&&placey!=='random'){placey=artisan.analyzeValue(placey,target_height);}
if(line_width&&line_width!=='random'){line_width=artisan.analyzeValue(line_width,target_width);}
if(shadow_blur!=='random'){shadow_blur=parseInt(shadow_blur,10);}
shadow_offset_x=parseInt(shadow_offset_x,10);shadow_offset_y=parseInt(shadow_offset_y,10);if(!placex){placex=10;}else if(placex==='random'){placex=artisan.randomize(0,target_width);}
...
break;default:}},drawRectangle:function(target,start_x,start_y,width,height,fill_color,line_width,stroke_color,alpha,shadow_blur,shadow_color,shadow_offset_x,shadow_offset_y){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;if(start_x&&start_x!=='random'){start_x=artisan.analyzeValue(start_x,target_width);}
if(start_y&&start_y!=='random'){start_y=artisan.analyzeValue(start_y,target_height);}
if(width&&width!=='random'){width=artisan.analyzeValue(width,target_width);}
if(height&&height!=='random'){height=artisan.analyzeValue(height,target_height);}
if(line_width&&line_width!=='random'){line_width=artisan.analyzeValue(line_width,target_width);}
if(shadow_blur!=='random'){shadow_blur=parseInt(shadow_blur,10);}
shadow_offset_x=parseInt(shadow_offset_x,10);shadow_offset_y=parseInt(shadow_offset_y,10);if(!start_x){start_x=10;}else if(start_x==='random'){start_x=artisan.randomize(0,target_width);}
...
break;default:}},drawImage:function(target,src,placex,placey,width,height,alpha,fill_color,line_width,stroke_color,shadow_blur,shadow_color,shadow_offset_x,shadow_offset_y){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;if(src){if(placex&&placex!=='random'){placex=artisan.analyzeValue(placex,target_width);}
if(placey&&placey!=='random'){placey=artisan.analyzeValue(placey,target_height);}
if(shadow_blur!=='random'){shadow_blur=parseInt(shadow_blur,10);}
shadow_offset_x=parseInt(shadow_offset_x,10);shadow_offset_y=parseInt(shadow_offset_y,10);if(width&&width!=='random'){width=artisan.analyzeValue(width,target_width);}
if(height&&height!=='random'){height=artisan.analyzeValue(height,target_height);}
if(line_width&&line_width!=='random'){line_width=artisan.analyzeValue(line_width,target_width);}
...
if(!shadow_offset_x||isNaN(shadow_offset_x)){shadow_offset_x=0;}
if(!shadow_offset_y||isNaN(shadow_offset_y)){shadow_offset_y=0;}
var format=artisan.interpretElement(target);switch(format){case"CANVAS":var context=artisan.findContext(target);var this_image=new Image();this_image.src=src;this_image.style.opacity=alpha;this_image.style.MozOpacity=alpha;this_image.onload=function(){context.shadowOffsetX=shadow_offset_x;context.shadowOffsetY=shadow_offset_y;context.shadowBlur=shadow_blur;context.shadowColor=shadow_color;context.strokeStyle=stroke_color;context.fillStyle=fill_color;context.drawImage(this_image,placex,placey,width,height);};break;default:}}},drawPath:function(target,path,line_width,line_color,fill_color){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;var cp1x,cp1y,cp2x,cp2y,cur_x,cur_y;var context=artisan.findContext(target);if(!path){path=[[0,0]];}
...
var starting_point_x=path[0][0];if(starting_point_x!=0&&starting_point_x){starting_point_x=artisan.analyzeValue(starting_point_x,target_width);}
var starting_point_y=path[0][1];if(starting_point_y!=0&&starting_point_y){starting_point_y=artisan.analyzeValue(starting_point_y,target_height);}
context.beginPath();context.moveTo(starting_point_x,starting_point_y);for(p=1;p<path.length;p++){var point_x=path[p][0];point_x=artisan.analyzeValue(point_x,target_width);var point_y=path[p][1];point_y=artisan.analyzeValue(point_y,target_height);if(!path[p][2]){context.lineTo(point_x,point_y);}else if(path[p][2]==='bezier'){cp1x=path[p][3];cp1y=path[p][4];cp2x=path[p][5];cp2y=path[p][6];context.bezierCurveTo(cp1x,cp1y,cp2x,cp2y,point_x,point_y);}else if(path[p][2]==='quadratic'){cp1x=cur_x+2.0/3.0*(path[p][3]-cur_x);cp1y=cur_y+2.0/3.0*(path[p][4]-cur_y);cp2x=cp1x+(point_x-cur_x)/3.0;cp2y=cp1y+(point_y-cur_y)/3.0;context.bezierCurveTo(cp1x,cp1y,cp2x,cp2y,point_x,point_y);}
cur_x=point_x;cur_y=point_y;}
if(line_width!==0&&line_color!==''){line_width=artisan.analyzeValue(line_width,target_width);context.lineWidth=line_width;context.stroke();}
if(fill_color!==''){context.fillStyle=fill_color;context.fill();}
context.closePath();},drawLine:function(target,start_x,start_y,end_x,end_y,line_width,line_color,type,cp1_x,cp1_y,cp2_x,cp2_y){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;if(start_x!=='random'&&start_x){start_x=artisan.analyzeValue(start_x,target_width);}
if(start_y!=='random'&&start_y){start_y=artisan.analyzeValue(start_y,target_height);}
if(start_x==='random'){start_x=artisan.randomize(0,target_width);}else if(!start_x||isNaN(start_y)){start_x='';}
if(start_y==='random'){start_y=artisan.randomize(0,target_height);}else if(!start_y||isNaN(start_y)){start_y='';}
if(end_x!=='random'&&end_x){end_x=artisan.analyzeValue(end_x,target_width);}
if(end_y!=='random'&&end_y){end_y=artisan.analyzeValue(end_y,target_height);}
...
if(cp1_x!=='random'&&cp1_x){cp1_x=artisan.analyzeValue(cp1_x,target_width);}
if(cp1_y!=='random'&&cp1_y){cp1_y=artisan.analyzeValue(cp1_y,target_height);}
...
if(cp2_x!=='random'&&cp2_x){cp2_x=artisan.analyzeValue(cp2_x,target_width);}
if(cp2_y!=='random'&&cp2_y){cp2_y=artisan.analyzeValue(cp2_y,target_height);}
...
if(line_width!=='random'&&line_width){line_width=artisan.analyzeValue(line_width,target_width);}
if(!line_width||isNaN(line_width)||line_width===null){line_width=0;}
...
if(line_width!==0&&line_color!==''){context.strokeStyle=line_color;context.lineWidth=line_width;context.stroke();}},closePath:function(target){var context=artisan.findContext(target);context.closePath();},drawText:function(target,place_x,place_y,text,text_color,weight,size,font,align,alpha,line_width,line_color,baseline){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;if(place_x!=='random'&&place_x){place_x=artisan.analyzeValue(place_x,target_width);}
if(place_y!=='random'&&place_y){place_y=artisan.analyzeValue(place_y,target_height);}
if(place_x==='random'){place_x=artisan.randomize(0,target_width);}else if(!place_x||isNaN(place_x)){place_x=0;}
if(place_y==='random'){place_y=artisan.randomize(0,target_height);}else if(!place_y||isNaN(place_y)){place_y=0;}
if(!text){text='';}
if(!text_color){text_color='#000000';}else if(text_color==='random'){chosen_color=artisan.randomize(0,default_colors.length);text_color=default_colors[chosen_color];}else if(text_color.constructor.toString().indexOf("Array")!==-1){chosen_color=artisan.randomize(0,text_color.length);text_color=text_color[chosen_color];}
if(!weight){weight='';}
if(size!=='random'&&size){size=artisan.analyzeValue(size,target_width)+'px';}else if(size==='random'){size=artisan.randomize(0,190)+'px';}else if(!size){size='15px';}
...
var old_history=stacks[stack][layer][history].toString();var new_history=current_history.toString();if(new_history!==old_history){stacks[stack][layer][history].push(current_history);}},randomize:function(l,h){var generated=parseInt(l+(Math.random()*(h-l)),10);return generated;},convertToPercentage:function(value,maximum){var percentage=(100/maximum)*value;return percentage;},convertToPixels:function(value,maximum){var pixel=maximum*(value/100);return pixel;},rotateCanvas:function(target,amount){if(amount==='reset'){amount=global_rotate*-1;}else if(amount==='random'){amount=artisan.randomize(-1,360);}
...
context.rotate((Math.PI*2)/div);},clearCanvas:function(target){var targeted=document.getElementById(target);var target_width=targeted.width;var target_height=targeted.height;var context=artisan.findContext(target);context.clearRect(0,0,target_width,target_height);},resetDefaults:function(context){context.globalAlpha=1;},analyzeValue:function(measurement,scale_measure){var measurement=measurement.toString();if(!!(measurement.match('%'))){measurement=parseFloat(measurement);measurement=artisan.convertToPixels(measurement,scale_measure);}else if(!!(measurement.match('px'))){measurement=parseInt(measurement);}else{measurement=parseInt(measurement);}
return measurement;}};})(this);artisan.create.stack();