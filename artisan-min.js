...
var stacks=[];var color_schemes=[];var default_colors=['#AC4A08','#6C88A1','#D9DCC6','#EB9E1A','#FBCC37'];var stack_limit=5;var layer_limit=5;var data_set_limit=5;var history_limit=300;var contexts=[];var global_rotate=0;var current_history=[];return{define:{canvas:function(){},stack:function(){var colors=[];},defaultColors:function(new_default){default_colors=new_default;}},set:{stackLimit:function(limit){stack_limit=limit;},layerLimit:function(limit){layer_limit=limit;},dataSetLimit:function(limit){data_set_limit=limit;},historyLimit:function(limit){history_limit=limit;}},setupCanvas:function(target){var canvas=document.getElementById(target);var context=canvas.getContext('2d');var this_context_pair=[target,context];contexts.push(this_context_pair);},create:{canvas:function(parent,width,height){},stack:function(){var new_stack=[];var new_layer=[];var new_history=[];new_layer.push(new_history);new_stack.push(new_layer);stacks.push(new_stack);if(stacks.length>stack_limit){stacks.splice(0,1);}},layer:function(stack){if(!stack||isNaN(stack)){stack=0;}
...
var new_history=[];stacks[stack][layer].push(new_history);if(stacks[stack][layer].length>history_limit){stacks[stack][layer].splice(0,1);}}},clear:{layer:function(stack,layer){if(!stack||isNaN(stack)){stack=0;}
if(!layer||isNaN(layer)){layer=0;}
...
var old_history=stacks[stack][layer][history].toString();var new_history=current_history.toString();if(new_history!==old_history){stacks[stack][layer][history].push(current_history);if(stacks[stack][layer].length>history_limit){stacks[stack][layer][history].splice(1,1);}}},randomize:function(l,h){var generated=parseInt(l+(Math.random()*(h-l)),10);return generated;},convertToPercentage:function(value,maximum){var percentage=(100/maximum)*value;return percentage;},convertToPixels:function(value,maximum){var pixel=maximum*(value/100);return pixel;},rotateCanvas:function(target,amount){if(amount==='reset'){amount=global_rotate*-1;}else if(amount==='random'){amount=artisan.randomize(-1,360);}
